...
import { require_react_is } from "./react-is-B7Gewe8w.js";
...
function isPlainObject(value) {
	if (!isObjectLike_default(value) || _baseGetTag_default(value) != objectTag$3) return false;
...
function hashHas(key) {
	var data = this.__data__;
	return _nativeCreate_default ? data[key] !== void 0 : hasOwnProperty$6.call(data, key);
}
...
var _defineProperty_default = defineProperty$1;
...
function cloneDeep(value) {
	return _baseClone_default(value, CLONE_DEEP_FLAG | CLONE_SYMBOLS_FLAG$1);
}
...
var require_react_fast_compare = /* @__PURE__ */ __commonJS({ "node_modules/react-fast-compare/index.js": ((exports, module) => {
	...
	function equal(a, b) {
		...
		if (a && b && typeof a == "object" && typeof b == "object") {
			...
			var keys$1 = keyList(a);
			length = keys$1.length;
			if (length !== keyList(b).length) return false;
			for (i = length; i-- !== 0;) if (!hasProp.call(b, keys$1[i])) return false;
...
function clone(value) {
	return _baseClone_default(value, CLONE_SYMBOLS_FLAG);
}
...
function toPath(value) {
	if (isArray_default(value)) return _arrayMap_default(value, _toKey_default);
	return isSymbol_default(value) ? [value] : _copyArray_default(_stringToPath_default(toString_default(value)));
}
...
var require_hoist_non_react_statics_cjs = /* @__PURE__ */ __commonJS({ "node_modules/hoist-non-react-statics/dist/hoist-non-react-statics.cjs.js": ((exports, module) => {
	...
	*/
	...
	function hoistNonReactStatics$1(targetComponent, sourceComponent, blacklist) {
		if (typeof sourceComponent !== "string") {
			if (objectPrototype) {
				...
				if (inheritedComponent && inheritedComponent !== objectPrototype) hoistNonReactStatics$1(targetComponent, inheritedComponent, blacklist);
			}
	...
	module.exports = hoistNonReactStatics$1;
}) });
...
function useFormikContext() {
	...
	!formik && tiny_warning_esm_default(false, "Formik context is undefined, please verify you are calling useFormikContext() as child of a <Formik> component.");
	return formik;
}
/** @private is the value an empty array? */
var isEmptyArray = function isEmptyArray$1(value) {
	return Array.isArray(value) && value.length === 0;
};
/** @private is the given object a Function? */
var isFunction = function isFunction$2(obj) {
	return typeof obj === "function";
};
/** @private is the given object an Object? */
var isObject = function isObject$2(obj) {
	return obj !== null && typeof obj === "object";
};
/** @private is the given object an integer? */
var isInteger = function isInteger$1(obj) {
...
var isNaN$1 = function isNaN$2(obj) {
	return obj !== obj;
};
/** @private Does a React component have exactly 0 children? */
var isEmptyChildren = function isEmptyChildren$1(children) {
	return import_react.Children.count(children) === 0;
};
/** @private is the given object/value a promise? */
var isPromise = function isPromise$1(value) {
	return isObject(value) && isFunction(value.then);
};
/** @private is the given object/value a type of synthetic event? */
var isInputEvent = function isInputEvent$1(value) {
...
function setIn(obj, path, value) {
	...
	for (; i < pathArray.length - 1; i++) {
		...
		if (currentObj && (isObject(currentObj) || Array.isArray(currentObj))) resVal = resVal[currentPath] = clone_default(currentObj);
		else {
...
function useFormik(_ref) {
	...
	(0, import_react.useEffect)(function() {
		!(typeof isInitialValid === "undefined") && tiny_warning_esm_default(false, "isInitialValid has been deprecated and will be removed in future versions of Formik. Please use initialErrors or validateOnMount instead.");
	}, []);
	(0, import_react.useEffect)(function() {
	...
	*/
	...
	var resetForm = (0, import_react.useCallback)(function(nextState) {
		...
		var dispatchFn = function dispatchFn$1() {
			dispatch({
				type: "RESET_FORM",
				payload: {
	...
	var setFieldValue = useEventCallback(function(field, value, shouldValidate) {
		...
		return (shouldValidate === void 0 ? validateOnChange : shouldValidate) ? validateFormWithHighPriority(setIn(state.values, field, value)) : Promise.resolve();
	});
	...
	var handleSubmit = useEventCallback(function(e) {
		...
		if (typeof document !== "undefined") {
			...
			if (activeElement !== null && activeElement instanceof HTMLButtonElement) !(activeElement.attributes && activeElement.attributes.getNamedItem("type")) && tiny_warning_esm_default(false, "You submitted a Formik form using a button with an unspecified `type` attribute.  Most browsers default button elements to `type=\"submit\"`. If this is not a submit button, please add `type=\"button\"`.");
		}
	...
	var getFieldHelpers = (0, import_react.useCallback)(function(name) {
		return {
			...
			setTouched: function setTouched$1(value, shouldValidate) {
...
function Formik(props) {
	...
	(0, import_react.useEffect)(function() {
		props.render && tiny_warning_esm_default(false, "<Formik render> has been deprecated and will be removed in future versions of Formik. Please use a child callback function instead. To get rid of this warning, replace <Formik render={(props) => ...} /> with <Formik>{(props) => ...}</Formik>");
	}, []);
...
function prepareDataForValidation(values) {
	...
	for (var k in values) if (Object.prototype.hasOwnProperty.call(values, k)) {
		...
		if (Array.isArray(values[key]) === true) data[key] = values[key].map(function(value) {
			if (Array.isArray(value) === true || isPlainObject_default(value)) return prepareDataForValidation(value);
			else return value !== "" ? value : void 0;
		});
		else if (isPlainObject_default(values[key])) data[key] = prepareDataForValidation(values[key]);
...
function arrayMerge(target, source, options) {
	...
	source.forEach(function merge(e, i) {
		if (typeof destination[i] === "undefined") destination[i] = options.clone !== false && options.isMergeableObject(e) ? es_default(Array.isArray(e) ? [] : {}, e, options) : e;
		else if (options.isMergeableObject(e)) destination[i] = es_default(target[i], e, options);
...
function useField(propsOrFieldName) {
	...
	!formik && tiny_warning_esm_default(false, "useField() / <Field /> must be used underneath a <Formik> component or withFormik() higher order component");
	!fieldName && tiny_warning_esm_default(false, "Invalid field name. Either pass `useField` a string or an object containing a `name` key.");
...
function Field(_ref) {
	...
	(0, import_react.useEffect)(function() {
		render && tiny_warning_esm_default(false, "<Field render> has been deprecated and will be removed in future versions of Formik. Please use a child callback function instead. To get rid of this warning, replace <Field name=\"" + name + "\" render={({field, form}) => ...} /> with <Field name=\"" + name + "\">{({field, form, meta}) => ...}</Field>");
		is && children && isFunction(children) && tiny_warning_esm_default(false, "You should not use <Field as> and <Field children> as a function in the same <Field> component; <Field as> will be ignored.");
		component && children && isFunction(children) && tiny_warning_esm_default(false, "You should not use <Field component> and <Field children> as a function in the same <Field> component; <Field component> will be ignored.");
		render && children && !isEmptyChildren(children) && tiny_warning_esm_default(false, "You should not use <Field render> and <Field children> in the same <Field> component; <Field children> will be ignored");
	}, []);
	...
	return function createFormik(Component$1) {
		...
		*/
		var C = /* @__PURE__ */ function(_React$Component) {
			_inheritsLoose(C$1, _React$Component);
			function C$1() {
				var _this;
				for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) args[_key] = arguments[_key];
...
function connect(Comp) {
	var C = function C$1(props) {
		return (0, import_react.createElement)(FormikConsumer, null, function(formik) {
			!formik && tiny_warning_esm_default(false, "Formik context is undefined, please verify you are rendering <Form>, <Field>, <FastField>, <FieldArray>, or your custom context-using component as a child of a <Formik> component. Component name: " + Comp.name);
...
* Some array helpers!
*/
var move = function move$1(array, from, to) {
...
var swap = function swap$1(arrayLike, indexA, indexB) {
...
var insert = function insert$1(arrayLike, index, value) {
	var copy = copyArrayLike(arrayLike);
	copy.splice(index, 0, value);
	return copy;
};
var replace = function replace$1(arrayLike, index, value) {
	var copy = copyArrayLike(arrayLike);
	copy[index] = value;
	return copy;
};
var copyArrayLike = function copyArrayLike$1(arrayLike) {
	if (!arrayLike) return [];
...
var createAlterationHandler = function createAlterationHandler$1(alteration, defaultFunction) {
	var fn = typeof alteration === "function" ? alteration : defaultFunction;
	return function(data) {
		if (Array.isArray(data) || isObject(data)) {
			var clone$1 = copyArrayLike(data);
			return fn(clone$1);
		}
		return data;
	};
...
var FieldArrayInner = /* @__PURE__ */ function(_React$Component) {
	_inheritsLoose(FieldArrayInner$1, _React$Component);
	function FieldArrayInner$1(props) {
	...
	var _proto = FieldArrayInner$1.prototype;
	...
	_proto.render = function render() {
		...
		var _this$props2 = this.props, component = _this$props2.component, render$1 = _this$props2.render, children = _this$props2.children, name = _this$props2.name, _this$props2$formik = _this$props2.formik, restOfFormik = _objectWithoutPropertiesLoose(_this$props2$formik, ["validate", "validationSchema"]);
		var props = _extends({}, arrayHelpers, {
			form: restOfFormik,
			name
		});
		return component ? (0, import_react.createElement)(component, props) : render$1 ? render$1(props) : children ? typeof children === "function" ? children(props) : !isEmptyChildren(children) ? import_react.Children.only(children) : null : null;
	};
	return FieldArrayInner$1;
}(import_react.Component);
...
var ErrorMessage = /* @__PURE__ */ connect(/* @__PURE__ */ function(_React$Component) {
	...
	_proto.render = function render() {
		var _this$props = this.props, component = _this$props.component, formik = _this$props.formik, render$1 = _this$props.render, children = _this$props.children, name = _this$props.name, rest = _objectWithoutPropertiesLoose(_this$props, [
			"component",
			"formik",
			"render",
			"children",
		...
		return !!touch && !!error ? render$1 ? isFunction(render$1) ? render$1(error) : null : children ? isFunction(children) ? children(error) : null : component ? (0, import_react.createElement)(component, rest, error) : error : null;
	};
...
var FastField = /* @__PURE__ */ connect(/* @__PURE__ */ function(_React$Component) {
	...
	function FastFieldInner(props) {
		...
		render && tiny_warning_esm_default(false, "<FastField render> has been deprecated. Please use a child callback function instead: <FastField name={" + name + "}>{props => ...}</FastField> instead.");
		component && render && tiny_warning_esm_default(false, "You should not use <FastField component> and <FastField render> in the same <FastField> component; <FastField component> will be ignored");
		is && children && isFunction(children) && tiny_warning_esm_default(false, "You should not use <FastField as> and <FastField children> as a function in the same <FastField> component; <FastField as> will be ignored.");
		component && children && isFunction(children) && tiny_warning_esm_default(false, "You should not use <FastField component> and <FastField children> as a function in the same <FastField> component; <FastField component> will be ignored.");
		render && children && !isEmptyChildren(children) && tiny_warning_esm_default(false, "You should not use <FastField render> and <FastField children> in the same <FastField> component; <FastField children> will be ignored");
		return _this;
	}
	...
	_proto.render = function render() {
		var _this$props = this.props, name = _this$props.name, render$1 = _this$props.render, is = _this$props.as, children = _this$props.children, component = _this$props.component, formik = _this$props.formik, props = _objectWithoutPropertiesLoose(_this$props, [
			"validate",
			"name",
			"render",
			"as",
		...
		if (render$1) return render$1(bag);
