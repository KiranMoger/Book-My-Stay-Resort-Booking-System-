...
function dehydrateQuery(query, serializeData, shouldRedactErrors) {
	...
	return {
		...
		...query.state.status === "pending" && { promise },
...
var QueriesObserver = class extends Subscribable {
	...
	setQueries(queries, options) {
		...
		notifyManager.batch(() => {
			...
			if (hasStructuralChange) this.#observers = newObservers;
	...
	#notify() {
		if (this.hasListeners()) {
			...
			if (previousResult !== newResult) notifyManager.batch(() => {
				this.listeners.forEach((listener) => {
...
function streamedQuery({ streamFn, refetchMode = "reset", reducer = (items, chunk) => addToEnd(items, chunk), initialValue = [] }) {
	return async (context) => {
		...
		for await (const chunk of stream) {
			...
			if (!isRefetch || refetchMode !== "replace") context.client.setQueryData(context.queryKey, (prev) => reducer(prev === void 0 ? initialValue : prev, chunk));
			result = reducer(result, chunk);
		}
		if (isRefetch && refetchMode === "replace" && !context.signal.aborted) context.client.setQueryData(context.queryKey, result);
...
//#endregion
export { CancelledError, HydrationBoundary, InfiniteQueryObserver, IsRestoringProvider, Mutation, MutationCache, MutationObserver, QueriesObserver, Query, QueryCache, QueryClient, QueryClientContext, QueryClientProvider, QueryErrorResetBoundary, QueryObserver, dataTagErrorSymbol, dataTagSymbol, defaultScheduler, defaultShouldDehydrateMutation, defaultShouldDehydrateQuery, dehydrate, focusManager, hashKey, hydrate, infiniteQueryOptions, isCancelledError, isServer, keepPreviousData, matchMutation, matchQuery, mutationOptions, noop, notifyManager, onlineManager, partialMatchKey, queryOptions, replaceEqualDeep, shouldThrowError, skipToken, streamedQuery, timeoutManager, unsetMarker, useInfiniteQuery, useIsFetching, useIsMutating, useIsRestoring, useMutation, useMutationState, usePrefetchInfiniteQuery, usePrefetchQuery, useQueries, useQuery, useQueryClient, useQueryErrorResetBoundary, useSuspenseInfiniteQuery, useSuspenseQueries, useSuspenseQuery };
//# sourceMappingURL=modern-z_NTRfsY.js.map